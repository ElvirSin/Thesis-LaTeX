...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\linewidth]{Bilder/A40_MenschUML}
	\caption{UML Diagramm der Skripte TrackerAssignment und CalibrationController, eigene Abbildung}
	\label{fig:MenschUML}
\end{figure}
...
\newline
Durch das Tracking von bis zu zehn Körperteilen (Beide Füße, beide Knie, das Steißbein, beide Hände, beide Ellenbogen und der Kopf), in Verbindung mit der Möglichkeit das Menschmodell an verschiedene Körpergrößen durch Kalibrierung anpassen zu können, wird die in Kapitel \ref{sec:AnforderungenKonzept} gestellte Anforderung an die Genauigkeit erfüllt. Des Weiteren wird dadurch, dass die Abbildung der Bewegungen auf den virtuellen Klon in nahe zu Echtzeit stattfindet, die in Kapitel \ref{sec:AnforderungenKonzept} gestellte Anforderung der Echtzeit ebenfalls erfüllt. Außerdem ist anzumerken, dass aufgrund des modularen Aufbaus des Menschmodells zukünftige Anpassungen und Erweiterungen ermöglicht werden und somit die in Kapitel \ref{sec:AnforderungenKonzept} geforderte Modularität gewährleistet wird. Schließlich ist wichtig hervorzugehen, dass das entstandene Menschmodell unabhängig von seiner Umgebung ist und somit in jeder beliebigen virtuellen Umgebung eingesetzt werden kann. Folglich wird auch die Anforderung an die Interoperabilität aus Kapitel \ref{sec:AnforderungenKonzept} erfüllt.
\newline
Zusammenfassend lässt sich sagen, dass das entstandene Menschmodell aus dem CameraRig (der Kamera), dem Dummy (dem eigentliches Modell), den HumanScripts (den Scripten) und den Targets (den Verfolgungszielen) besteht (Vgl. Abbildung \ref{fig:UnityOverview}).
%--------------------------------------------------------------------------------------------------
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\linewidth]{Bilder/A44_InteraktionsBeispiel}
...
Im Folgenden wird zunächst das Grundgerüst der Interaktionsschnittstelle erklärt, bevor die zusätzlichen Funktionalitäten erläutert werden. Es ist anzumerken, dass die zusätzlichen Funktionalitäten eingefügt wurden, um ein Anwendungsbeispiel für die Interaktionsschnittstelle zu schaffen und somit ihren Nutzen hervorzuheben.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.6\linewidth]{Bilder/A42_CanvasPointer}
	\includegraphics[width=0.6\linewidth]{Bilder/A43_PhysicsPointer}
...
\subsection{Zusammenfassung der Interaktionsschnittstelle}\label{sec:ZusammenfassungInteraktion}
todo zusammenfassung
\newline
todo inwidern es die anforderungen erfüllt
\newline
todo zusammenfassung anhand abbildung unityoverview